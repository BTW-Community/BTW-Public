...
public class FCBlockSpike extends Block
{
    ...
    public boolean RenderBlock( RenderBlocks renderBlocks, int i, int j, int k )
    {
    	...
    	FCModelBlock transformedCenterModel = m_model.MakeTemporaryCopy();
    		...
    		( iFacing == 1 && blockAccess.getBlockId( i, j + 1, k ) != blockID ) )
    	{
        	...
        	{
        		m_model.m_modelBase.MakeTemporaryCopyOfPrimitiveList( transformedCenterModel );
        	}
    	}
    	...
    	if ( iFacing == 1 )
    	{
			...
			{
				m_model.m_modelBall.MakeTemporaryCopyOfPrimitiveList( transformedCenterModel );
			}
    	}
    	else
    	{
			...
			{
				if ( FCUtilsWorld.IsBlockRestingOnThatBelow( blockAccess, i, j + 1, k ) )
				{
					m_model.m_modelHolder.MakeTemporaryCopyOfPrimitiveList( transformedCenterModel );
				}
    	...
    	if ( iFacing == 1 )
    	{
    		transformedCenterModel.TiltToFacingAlongJ( 0 );
    	}
    	...
    	if ( transformedSupportsModel != null )
    	{
			...
			m_model.m_modelCenterBrace.MakeTemporaryCopyOfPrimitiveList( transformedCenterModel );			
    	}
