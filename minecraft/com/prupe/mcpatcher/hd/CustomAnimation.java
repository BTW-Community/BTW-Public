...
public class CustomAnimation implements Comparable<CustomAnimation> {
    ...
    static {
        TexturePackChangeHandler.register(new TexturePackChangeHandler(MCPatcherUtils.EXTENDED_HD, 1) {
            ...
            public void afterChange() {
                if (enable) {
                    for (FakeResourceLocation resource : ResourceList.getInstance().listResources(TexturePackAPI.MCPATCHER_SUBDIR + "anim", ".properties", false)) {
    ...
    private static void checkPendingAnimations() {
        List<PropertiesFile> done = new ArrayList<PropertiesFile>();
        for (PropertiesFile properties : pending) {
    ...
    private static void addStrip(PropertiesFile properties) {
        ...
        if (x + w > dstWidth || y + h > dstHeight) {
            properties.error("%s dimensions x=%d,y=%d,w=%d,h=%d exceed %s size %dx%d",
                srcName, x, y, w, h, dstName, dstWidth, dstHeight
            );
            return;
        }
    ...
    private CustomAnimation(PropertiesFile properties, FakeResourceLocation srcName, FakeResourceLocation dstName, int mipmapLevel, int x, int y, int w, int h, ByteBuffer imageData, int numFrames) {
    ...
    public String toString() {
        return String.format("CustomAnimation{%s %s %dx%d -> %s%s @ %d,%d (%d frames)}",
            properties, srcName, w, h, dstName, (mipmapLevel > 0 ? "#" + mipmapLevel : ""), x, y, numFrames
        );
    }
