...
public class SkyRenderer {
        ...
        TexturePackChangeHandler.register(new TexturePackChangeHandler(MCPatcherUtils.BETTER_SKIES, 2) {
            ...
            public void afterChange() {
                if (enable) {
                    ...
                    if (world != null) {
                        getWorldEntry(world.provider.getWorldType());
                    }
                }
    ...
    private static class WorldEntry {
        ...
        private void loadSkies() {
            for (int i = -1; ; i++) {
                String path = "/environment/sky" + worldType + "/sky" + (i < 0 ? "" : String.valueOf(i)) + ".properties";
        ...
        private void loadCelestialObject(String objName) {
            ...
            String path = "/environment/sky" + worldType + "/" + objName + ".properties";
            FakeResourceLocation resource = TexturePackAPI.newMCPatcherResourceLocation(path);
        ...
        void renderAll(Tessellator tessellator) {
            if (unloadTextures) {
                Set<FakeResourceLocation> texturesNeeded = new HashSet<FakeResourceLocation>();
                for (Layer layer : skies) {
                    if (layer.prepare()) {
                        texturesNeeded.add(layer.texture);
                    }
                }
                Set<FakeResourceLocation> texturesToUnload = new HashSet<FakeResourceLocation>();
